...
| file | what it is |
|---|---|
| `evosim-v0_54_0.html` | the build. Single file, no build step, runs on a phone. Bump the filename every version — delete the superseded one once its results are captured in `LEDGER.md`. `evosim-v0_53_0.html` is kept alongside it as **the last build verified bit-identical to its predecessor at defaults**; v0.54 changes the arbiter's shape and is deliberately not RNG-identical, so v0.53 stays until v0.54's arms are scored. **Explicit deletion criterion:** delete `evosim-v0_53_0.html` once (a) `node check.js evosim-v0_54_0.html` passes (already true, all six stages) and (b) H4/H5/H6/H7 have been scored against the matched v0.54 control on the days 400–800 window and written into `LEDGER.md`, win or lose. If H5's HARM clause fires (survival under 50%), v0.54 is reverted and v0.53 becomes the build again — that is the case v0.53 is being kept for. |
